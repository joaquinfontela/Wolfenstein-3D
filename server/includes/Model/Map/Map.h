...
class Map {
 public:
  ...
  /*
   * [SERVER-SIDE] Applies damage to all players within radius of a certain center
   * 
  ...
  /*
   * [SERVER-SIDE] Moves item from one position to another, without the possibilty of notify of changes
   * 
   * @param Initial X Coordinate
   * @param Initial Y Coordinate
   ...
   * 
   * @return First hit player, nullptr otherwise. (Maybe consider using std::optional?)
   */
  Player* traceAttackFrom(Player* attacker, int range);

  /*
   * [SERVER-SIDE] Static method, increases the unique id counter for all items on the map and return the new id
   * 
