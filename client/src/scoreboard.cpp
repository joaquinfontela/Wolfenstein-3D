...
void ScoreBoard::renderText(const char* text, int rows, int column, int w, int h) {
  SDL_Rect rect = {.x = 5 * w / 48 + column * 15 * w / 48, .y = (h * rows) / 6, .w = w >> 3, .h = h >> 3 };
...
void ScoreBoard::draw() {
  ...
  this->scores.push_back(std::make_tuple(99,99));
  this->scores.push_back(std::make_tuple(100,100));
  int rows = this->scores.size();
  int height, width;
  SDL_Rect rect;
  while (!this->hasEnded()) {
    this->window->getWindowSize(&width, &height);
    rect = {.x = (3 * width) >> 3, .y = -(height >> 2) / 5, .w = width >> 2, .h = height >> 2};
